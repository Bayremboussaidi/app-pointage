...
export class TablefComponent implements OnInit {
  records: Record[] = [];
  personnelSummary: { idd: string; name: string; totalHours: string }[] = []; // ✅ Summary table data
  idd: string = '';
  date1: string = '';
  date2: string = '';
  errorMessage: string = '';
  ...
  generatePersonnelSummary() {
    ...
    this.personnelSummary = Array.from(summaryMap, ([idd, data]) => ({
      idd,
